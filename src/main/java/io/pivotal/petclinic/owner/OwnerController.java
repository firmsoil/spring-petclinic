...
class OwnerController {
    ...
    public OwnerController(OwnerRepository clinicService, PetRepository petRepository, VisitRepository visitRepository) {
    ...
    public ModelAndView showOwner(@PathVariable("ownerId") String ownerId) {
        ...
        mav.addObject("owner", getDisplayOwner(this.owners.findById(ownerId).get()));
        return mav;
    }

    private DisplayOwner getDisplayOwner(Owner owner) {
        DisplayOwner dOwner = new DisplayOwner(owner);
        List<Pet> pets = this.petRepository.findByOwnerId(owner.getId());
        pets.forEach(pet -> {
